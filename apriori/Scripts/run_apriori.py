...
filename = '..{0}Data{0}data.txt'.format(dir_sep)
minSup = 25
minConf = 60
maxRule = 4
...
status1 = run('..{0}Tools{0}apriori{1} -f"," -s{2} -v"[Sup. %S]" {3} apriori_1.txt'
...
if minConf > 0:
    ...
    status2 = run('..{0}Tools{0}apriori{1} -tr -f"," -n{2} -c{3} -s{4} -v"[Conf. %C,Sup. %S]" {5} apriori_2.txt'
